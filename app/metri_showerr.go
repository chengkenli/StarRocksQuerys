...
func showErrors(app string, db *gorm.DB) []showErr {
    ...
    sql := fmt.Sprintf("select %s from %s where %s >= date_sub(now(), interval %d minute) and user != 'root' and state='ERR' order by starttime desc", columns, util.Read.Schema.Auditops, starttime, minute)
    r := db.Raw(sql).Scan(&m)
    if r.Error != nil {
        return nil
    }
...
CanonicalForm:`, queryid)
    ...
    queryId := fmt.Sprintf(`<a href="javascript:void(0)" onclick="fetchAndRedirect('http://%s:%d/getstmtid?app=%s&id=%s')" class="plain-text-link" style="color: #333; text-decoration: none; font-weight: normal; cursor: pointer">%s</a>`,
        util.H.Ip, util.Read.Server.Port, app, queryid, queryid)
