...
func (engine *threadMap) showSubmitErr(c *gin.Context) {
	...
	c.JSON(http.StatusOK, sortByStartTimeDesc(showSubmitcancel(appid, db)))
}

func showSubmitcancel(app string, db *gorm.DB) []showErr {
	var errs []showErr

	var m []map[string]interface{}
	r := db.Raw("select * from information_schema.task_runs where state='FAILED'").Scan(&m)
	if r.Error != nil {
