...
function setDefaultTimes() {
  ...
  console.log("update time:", { begintime, endtime });
}
  ...
  document.getElementById("datetime-info").addEventListener("click", function () {
    ...
    document.getElementById("confirm-btn").addEventListener("click", function () {
      ...
      if (!startValue || !endValue) {
        alertMsg("请填写开始和结束时间", "warning");
        return;
      }

      if (!isValidDateTime(startValue) || !isValidDateTime(endValue)) {
        alertMsg("时间格式不正确，请使用 yyyy-mm-dd 时:分:秒 格式", "warning");
        return;
      }
      ...
      console.log("choice time:", { begintime, endtime });
    });
  });
});
