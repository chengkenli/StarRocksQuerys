function renderTable(data) {
  ...
  data.forEach((item, index) => {
    ...
    // 处理管理员显示
    if (item.admin) {
      userDisplay += ' <span class="badge bg-white text-success">管理员</span>';
    }
    // 处理二级重点账号显示
    if (item.point) {
      userDisplay += ' <span class="badge bg-white text-secondary">重点</span>';
    }
    // 处理短查询保障
    if (item.shortlist) {
      userDisplay += ' <span class="badge bg-white text-dark">短查询</span>';
    }
    // 处理白名单显示
    if (item.whitelist) {
      userDisplay += ' <span class="badge bg-white text-primary">白名单</span>';
    }
    // 处理追踪用户名
    ...
    row.innerHTML = `
                    ...
                    <td class="py-1" data-tooltip="${item.db}">${item.state}</td>
...
function setupDisconnectButtonHandler() {
  ...
  document.addEventListener("click", function (event) {
    ...
    if (killButton) {
      ...
      if (confirm(`(◔‿◔)确定要清理${userid}所有的连接吗？`)) {
        apiFetch("/disconnect", "POST", { user: userid })
          .then((data) => {
            console.info(data.message);
            alert(data.message);
            if (typeof fetchData === "function") {
